...
var JsonLdSerializer = function (rdf, options) {
  ...
  this.serialize = function (graph, callback) {
    ...
    var subjectIndex = function (s) {
      if (typeof subjects[s] === 'undefined') {
        jsonGraph.push({ '@id': s });
        subjects[s] = jsonGraph.length - 1;
      }
    ...
    graph.forEach(function (t) {
      var s = subjectIndex(t.subject.valueOf());
...
var RdfXmlParser = function (rdf, options) {
  ...
  this.process = function (toparse, callback, base, filter, done) {
    if (typeof toparse === 'string') {
      toparse = options.parseXml(toparse);
    }
    ...
    store.literal = function (value, language, type) { return rdf.createLiteral(value, language, type); };
